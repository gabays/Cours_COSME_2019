...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0" xpath-default-namespace="http://www.tei-c.org/ns/1.0">
    ...
    <xsl:template match="fw">
        <xsl:choose>
            <xsl:when test="@type = 'bookBinding'">
                <div class="fw"> [cahier] <xsl:apply-templates/> </div>
            </xsl:when>
            <xsl:when test="@type = 'catchword'">
                <div class="fw"> <xsl:apply-templates/> [Réclame] </div>
            </xsl:when>
            <xsl:when test="@type = 'pageNum'">
                <div class="fw"> [Page] <xsl:apply-templates/> </div>
            </xsl:when>
            <xsl:when test="@type = 'head'">
                <div class="fw"> [En-tête] <xsl:apply-templates/> </div>
            </xsl:when>
            <xsl:otherwise>
                <div class="fw">
                    <xsl:apply-templates/>
    ...
    <xsl:template match="div[@type = 'scene']//l">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <div class="verse"> <xsl:attribute name="id"> <xsl:value-of select="@xml:id"/> </xsl:attribute> [<xsl:value-of select="@n"/>] <xsl:apply-templates/> </div>
            </xsl:otherwise>
        </xsl:choose>
